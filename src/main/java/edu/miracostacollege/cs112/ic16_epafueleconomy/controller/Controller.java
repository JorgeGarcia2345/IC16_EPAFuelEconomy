package edu.miracostacollege.cs112.ic16_epafueleconomy.controller;

import edu.miracostacollege.cs112.ic16_epafueleconomy.model.Model;
import edu.miracostacollege.cs112.ic16_epafueleconomy.model.Vehicle;
...
public class Controller {
	...
	private ObservableList<String> initializeDistinctMakes()
	{
		...
		for(Vehicle vehicle :mAllVehiclesList)
			if (!distinctMakes.contains(vehicle.getMake()))
				distinctMakes.add(vehicle.getMake());
	...
	public ObservableList<Vehicle> filter(String make, double minMPG, double maxAnnualFuelCost) {
		...
		for (Vehicle vehicle : mAllVehiclesList) {
			if(("".equals(make) || vehicle.getMake().equals(make) ) && vehicle.getCombinedMPG() >= minMPG && vehicle.getAnnualFuelCost() <= maxAnnualFuelCost)
				mFilteredVehiclesList.add(vehicle);
		}
